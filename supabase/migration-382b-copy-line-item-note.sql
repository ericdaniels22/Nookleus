-- Issue #382 — Line-item notes: teach the copy RPCs to carry the note.
--
-- ORDERING IS LOAD-BEARING. This migration must run AFTER
-- migration-382a-line-item-note-columns.sql, which adds the `note` column to
-- estimate_line_items and invoice_line_items. These CREATE OR REPLACE bodies
-- read and write that column; calling them before the column exists raises
-- `column "note" does not exist` at runtime.
--
-- What changes vs the prior function bodies:
--   * apply_template_to_estimate (was migration-353): reads `note` from each
--     flat snapshot item and writes it onto the new estimate_line_items row.
--     Everything else is byte-identical to #353.
--   * convert_estimate_to_invoice (was migration-build67f): selects `note` from
--     each estimate_line_items row and writes it onto the new invoice_line_items
--     row. Everything else is byte-identical to #67f.
--
...
-- ============================================================================
-- convert_estimate_to_invoice — copy each line item's note onto the invoice.
-- Body is byte-identical to migration-build67f-convert-any-status.sql except
-- `note` is added to the line-item SELECT and INSERT.
-- ============================================================================
