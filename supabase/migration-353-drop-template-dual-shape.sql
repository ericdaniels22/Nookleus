-- Issue #353 — Drop the transitional dual-shape fallback from the apply RPC.
-- See docs/adr/0004-template-line-items-snapshot.md for the rationale.
--
-- Now that #352 has backfilled every estimate_templates.structure item into the
-- flat snapshot shape, apply_template_to_estimate no longer needs to resolve
-- anything from item_library or from the legacy `*_override` fields. This
-- rewrites the function (a pure body swap, no schema change) to read the flat
-- snapshot fields directly.
--
-- What changes vs migration-351:
--   * No item_library lookup at apply time. The function never reads the library;
--     `library_item_id` is still copied onto the line item as a soft breadcrumb,
--     but straight from the structure JSONB, not via a lookup.
--   * No `*_override` fallback rung. name/description/code/unit/quantity/
--     unit_price come only from the flat snapshot fields.
--   * No `broken_refs`. With no library lookup, apply can no longer hit a
--     dangling reference, so the return value drops broken_refs entirely and is
--     now just { section_count, line_item_count }.
--
-- What stays the same:
--   * The NOT-NULL insert defaults on estimate_line_items: description falls back
--     to '[unknown item]', quantity to 1, unit_price to 0 when the snapshot
--     leaves them null. These are insert-time floors for NOT-NULL columns, not
--     part of the dual-shape resolution being removed.
--   * The draft/empty/active guards, the sections + subsections walk, the
--     library_item_id breadcrumb, the opening/closing statement copy, and the
--     markup/discount/tax totals recompute — all identical to #351.
--
-- Depends on: #352 backfill having run everywhere. Applying a template that was
--             never backfilled (an item carrying only `*_override` values and no
--             flat fields) now loses those values to the NOT-NULL defaults — the
--             backfill is the prerequisite that makes dropping the fallback safe.
--
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Snapshot-only read (#353): every value comes straight from the flat
      -- fields. library_item_id is kept as a breadcrumb but never looked up.
      -- The COALESCE floors only backstop the NOT-NULL line-item columns.
