-- ============================================================================
-- Build 84: convert_estimate_to_invoice — reinstate the #382 note carry-over
-- ============================================================================
--
-- migration-build82b (#575) was authored against the build67f function body,
-- not the then-current one from migration-382b — so its CREATE OR REPLACE
-- silently REVERTED #382's note carry-over: converting an estimate dropped
-- every line item's note on the resulting invoice. (The same stale-base
-- mistake build82b itself fixed for markup: each CREATE OR REPLACE must be
-- diffed against the LATEST applied body, not the one it remembers.)
--
-- This body is byte-identical to migration-build82b with one change: `note`
-- is back in the line-item SELECT and INSERT, exactly as migration-382b had
-- it. Everything else — the Overhead/Profit copy + recompute (#575), the
-- any-status convert (#67f) — is unchanged.
--
-- Run AFTER build82b (or instead of it — this body supersedes it entirely).
-- Idempotent: plain CREATE OR REPLACE FUNCTION.
--
