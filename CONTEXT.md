...
but all one Sketch.) See [ADR 0025](docs/adr/0025-sketch-is-one-parametric-spine-feeding-the-estimate.md).
...
customer PDF, the estimate→invoice recompute) stay equipment-ignorant. The unit
price is then a **per-piece-per-day** rate. A row's billing is chosen by its
`pricing_mode` (`standard` | `pieces_days`); in `pieces_days` mode the derived
"N units for M days" note **owns the row's note slot** (the manual Note field is
