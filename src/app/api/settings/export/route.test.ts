...
describe("GET /api/settings/export — gated on access_settings (#107)", () => {
  it("returns 401 when unauthenticated", async () => {
    authed({ user: null });
    expect((await GET(new Request("http://test"), noParams)).status).toBe(401);
  });

  it("returns 403 when the caller lacks access_settings — the CSV dump never runs", async () => {
    authed({
      user: { id: "u" },
  ...
  it("passes the gate when the caller holds access_settings", async () => {
    ...
    expect(
      (await GET(new Request("http://test"), noParams)).status,
    ).not.toBe(403);
  });
