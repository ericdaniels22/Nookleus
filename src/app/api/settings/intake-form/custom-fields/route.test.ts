...
describe("GET /api/settings/intake-form/custom-fields — gated on access_settings (#107)", () => {
  it("returns 401 when unauthenticated", async () => {
    authed({ user: null });
    expect(
      (await GET(new Request("http://test?jobId=j-1"), noParams)).status,
    ).toBe(401);
  });
  ...
  it("returns custom fields when the caller holds access_settings", async () => {
    authed({
      user: { id: "u" },
      tables: memberTables({
        userId: "u",
