...
// GET /api/item-library/[id] — reading needs either `view_estimates` or
// `view_invoices` (admins auto-pass). Multi-key any-of `permission` rule.
export const GET = withRequestContext(
  { permission: ["view_estimates", "view_invoices"] },
  async (_request, ctx, { params }: { params: Promise<RouteParams> }) => {
...
// PUT /api/item-library/[id] — editing a library item needs the
// `manage_item_library` permission (admins auto-pass).
export const PUT = withRequestContext(
  { permission: "manage_item_library" },
  async (request, ctx, { params }: { params: Promise<RouteParams> }) => {
    const { id } = await params;
    const supabase = ctx.supabase;
...
// DELETE /api/item-library/[id] — deactivating a library item needs the
// `manage_item_library` permission (admins auto-pass).
